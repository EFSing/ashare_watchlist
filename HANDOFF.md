...
- implementation：branch/worktree=`codex/friday-volume-observation-only-backfill-v1` / `C:\awv`，PR #77 当前 head=`a61e77ff08eb9f90184f7280c7bf345df8e51de2`，OPEN/CLEAN/MERGEABLE。正常路径写入 `data/volume_observations/volume_observations_YYYYMMDD.json`；renderer 优先按 signal_id/代码读取，index/shadow 缺失不会遮蔽有效股票量能。
