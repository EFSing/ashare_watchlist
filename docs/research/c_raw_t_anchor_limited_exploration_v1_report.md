...
- 协议冻结 provenance：内容定稿 2026-09-24 12:12:35 +0800（SHA-256 `3aa6ffe2…e59e3`，此后未再改动）；
  全样本 outcome 运行产物生成于 13:57:22 +0800；冻结提交 `9616b49` 创建于 14:01:23 +0800，
  **晚于**该运行——本条冻结是“内容在读取收益前固定”，不是“提交早于 outcome”。
...
前缀不变性（抽样核验）：`PASS`，比较跨度 [120, 250, 500]，检查 400 个 (symbol, T)，不一致 0。
全历史对照探针：400 次中 2 次因重建价格非正被冻结纯函数判为非法输入。

预筛超集验证（抽样核验）：`PASS`，检查 2000 个被预筛拒绝的 (symbol, T)，未发现假阴性（0）。
这是文档化的抽样核验，**不声称**已穷尽证明“预筛接受集合 ⊇ 完整评估接受集合”。
...
枚举口径：`entry_candidate=True`，universe 为沪深 00/60 主板、ST 状态未核验；预筛只是必要条件的加速，
并在 2000 个被拒抽样上未发现假阴性；不声称穷尽超集证明。
